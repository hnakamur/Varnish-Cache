...
case SYM_VMOD: return("vmod");
case SYM_ACL: return("acl");
case SYM_SUB: return("sub");
case SYM_BACKEND: return("backend");
case SYM_PROBE: return("probe");
case SYM_WILDCARD: return("wildcard");
case SYM_OBJECT: return("object");
case SYM_METHOD: return("method");
# 45 "vcc_symb.c" 2

 default:
  vcc__ErrInternal(tl, __func__, 47);
...
vcc_AddSymbol(struct vcc *tl, const char *nb, int l, enum symkind kind)
{
 ...
 for ((sym) = (((&tl->symbols))->vtqh_first); (sym); (sym) = (((sym))->list.vtqe_next)) {
  if (sym->nlen != l)
   continue;
  ...
  vcc__ErrInternal(tl, __func__, 67);
  return (((void *)0));
 }
 do { (sym) = calloc(sizeof *(sym), 1); if ((sym) != ((void *)0)) (sym)->magic = (0x3368c9fb); } while (0);
 do { do { if (!((sym) != 0)) { VAS_Fail(__func__, "vcc_symb.c", 71, "(sym) != 0", VAS_ASSERT); } } while (0); } while (0);
 sym->name = malloc(l + 1L);
 do { do { if (!((sym->name) != 0)) { VAS_Fail(__func__, "vcc_symb.c", 73, "(sym->name) != 0", VAS_ASSERT); } } while (0); } while (0);
 memcpy(sym->name, nb, l);
 sym->name[l] = '\0';
 sym->nlen = l;
 do { if (((((sym))->list.vtqe_next) = (((&tl->symbols))->vtqh_first)) != ((void *)0)) (((&tl->symbols))->vtqh_first)->list.vtqe_prev = &(((sym))->list.vtqe_next); else (&tl->symbols)->vtqh_last = &(((sym))->list.vtqe_next); (((&tl->symbols))->vtqh_first) = (sym); (sym)->list.vtqe_prev = &(((&tl->symbols))->vtqh_first); } while (0);
 sym->kind = kind;
 return (sym);
}
...
VCC_GetSymbolTok(struct vcc *tl, const struct token *tok, enum symkind kind)
{
 ...
 if (sym == ((void *)0)) {
  ...
  do { do { if (!((sym) != 0)) { VAS_Fail(__func__, "vcc_symb.c", 104, "(sym) != 0", VAS_ASSERT); } } while (0); } while (0);
  sym->def_b = tok;
 }
 return (sym);
}
...
VCC_FindSymbol(struct vcc *tl, const struct token *t, enum symkind kind)
{
 ...
 do { if (!(t->tok == 132)) { VAS_Fail(__func__, "vcc_symb.c", 115, "t->tok == ID", VAS_ASSERT); } } while (0);
 for ((sym) = (((&tl->symbols))->vtqh_first); (sym); (sym) = (((sym))->list.vtqe_next)) {
  if (sym->kind == SYM_WILDCARD &&
     (t->e - t->b > sym->nlen) &&
     !memcmp(sym->name, t->b, sym->nlen)) {
   do { do { if (!((sym->wildcard) != 0)) { VAS_Fail(__func__, "vcc_symb.c", 120, "(sym->wildcard) != 0", VAS_ASSERT); } } while (0); } while (0);
