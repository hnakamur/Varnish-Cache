...
# 1 "<command-line>" 2
# 1 "vcc_backend_util.c"
# 31 "vcc_backend_util.c"
# 1 "../../config.h" 1
# 32 "vcc_backend_util.c" 2
...
# 38 "vcc_backend_util.c" 2
# 49 "vcc_backend_util.c"
struct fld_spec {
 const char *name;
 struct token *found;
};
...
vcc_FldSpec(struct vcc *tl, const char *first, ...)
{
 ...
 while (1) {
  ...
  if (p == ((void *)0))
   break;
  f[n++].name = p;
  do { if (!(n < 100)) { VAS_Fail(__func__, "vcc_backend_util.c", 77, "n < 100", VAS_ASSERT); } } while (0);
 }
 __builtin_va_end(ap);
 f[n++].name = ((void *)0);
...
vcc_IsField(struct vcc *tl, struct token **t, struct fld_spec *fs)
{
 ...
 do { vcc__Expect(tl, '.', 94); do { if ((tl)->err) return; } while (0); vcc_NextToken(tl); } while (0);
 do { vcc__Expect(tl, 132, 95); do { if ((tl)->err) return; } while (0);} while (0);
 t_field = tl->t;
 *t = t_field;
 vcc_NextToken(tl);
 do { vcc__Expect(tl, '=', 99); do { if ((tl)->err) return; } while (0); vcc_NextToken(tl); } while (0);

 for (; fs->name != ((void *)0); fs++) {
