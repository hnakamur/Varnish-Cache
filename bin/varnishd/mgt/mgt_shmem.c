...
# 46 "mgt/mgt_shmem.c" 2

# 1 "../../include/flopen.h" 1
# 34 "../../include/flopen.h"
int flopen(const char *, int, ...);
int fltest(int fd, pid_t *pid);
# 48 "mgt/mgt_shmem.c" 2
...
# 49 "mgt/mgt_shmem.c" 2
# 1 "../../include/vmb.h" 1
# 50 "mgt/mgt_shmem.c" 2
# 1 "../../include/vfil.h" 1
# 32 "../../include/vfil.h"
    ...
    const char *class, const char *type, const char *ident)
{
 ...
 do { do { if (!((p) != 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 80, "(p) != 0", VAS_ASSERT); } } while (0); } while (0);
 memcpy(p, ptr, size);
 if (heritage.vsm != ((void *)0)) {
  p = VSM_common_alloc(heritage.vsm, size, class, type, ident);
  do { do { if (!((p) != 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 84, "(p) != 0", VAS_ASSERT); } } while (0); } while (0);
  memcpy(p, ptr, size);
 }
}
# 97 "mgt/mgt_shmem.c"
...
vsm_n_check(void)
{
 ...
 do { do { if (!((fstat(fd, &st)) == 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 110, "(fstat(fd, &st)) == 0", VAS_ASSERT); } } while (0); } while (0);
 if (!((((st.st_mode)) & 0170000) == (0100000))) {
  ...
  if (i < 0) {
   ...
   if (pread(fd, &vsmh, sizeof vsmh, 0) == sizeof vsmh) {
    vsmh.alloc_seq = 0;
    do { if (!(sizeof vsmh == pwrite(fd, &vsmh, sizeof vsmh, 0))) { VAS_Fail(__func__,
 "mgt/mgt_shmem.c"
# 127 "mgt/mgt_shmem.c"
    ,
 128
# 127 "mgt/mgt_shmem.c"
    , "sizeof vsmh == pwrite(fd, &vsmh, sizeof vsmh, 0)", VAS_ASSERT); } } while (0)
                                          ;
   }
   retval = 0;
  } else {
...
vsm_zerofile(const char *fn, ssize_t size)
{
 ...
 fd = flopen(fn, 02 | 0100 | 0200 | 04000, 0640);
 if (fd < 0) {
 ...
 VJ_fix_vsm_file(fd);
 flags = fcntl(fd, 3);
 do { if (!(flags != -1)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 161, "flags != -1", VAS_ASSERT); } } while (0);
 flags &= ~04000;
 do { do { if (!((fcntl(fd, 4, flags)) == 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 163, "(fcntl(fd, 4, flags)) == 0", VAS_ASSERT); } } while (0); } while (0);
...
mgt_shm_size(void)
{
 ...
 ps = getpagesize();
 size = (((size)+((ps)-1))&(~((uintptr_t)(ps)-1UL)));
 return (size);
}
...
mgt_SHM_Create(void)
{
 ...
 do { do { if (!((heritage.vsm) == 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 195, "(heritage.vsm) == 0", VAS_ASSERT); } } while (0); } while (0);
 size = mgt_shm_size();

 do { do { if (!(snprintf(fnbuf, sizeof fnbuf, "%s.%jd", "_.vsm", (intmax_t)getpid()) < sizeof fnbuf)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 198, "snprintf(fnbuf, sizeof fnbuf, \"%s.%jd\", \"_.vsm\", (intmax_t)getpid()) < sizeof fnbuf", VAS_ASSERT); } } while (0); } while (0);
 ...
 p = (void *)mmap(((void *)0), size,
     0x1|0x2,
     0 | 0 | 0x01,
     vsm_fd, 0);

 do { do { if (!((close(vsm_fd)) == 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 211, "(close(vsm_fd)) == 0", VAS_ASSERT); } } while (0); } while (0);

 if (p == ((void *) -1)) {
  fprintf(stderr, "Mmap error %s: %s\n", fnbuf, strerror((*__errno_location ())));
  exit(1);
 }
 ...
 heritage.param = VSM_common_alloc(heritage.vsm,
     sizeof *heritage.param, "Params", "", "");
 do { do { if (!((heritage.param) != 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 230, "(heritage.param) != 0", VAS_ASSERT); } } while (0); } while (0);
 *heritage.param = mgt_param;

 heritage.panic_str_len = 64 * 1024;
 heritage.panic_str = VSM_common_alloc(heritage.vsm,
     heritage.panic_str_len, "Panic", "", "");
 do { do { if (!((heritage.panic_str) != 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 236, "(heritage.panic_str) != 0", VAS_ASSERT); } } while (0); } while (0);


 VSC_C_mgt = VSM_common_alloc(heritage.vsm,
     sizeof *VSC_C_mgt, "Stat", VSC_type_mgt, "");
 do { do { if (!((VSC_C_mgt) != 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 241, "(VSC_C_mgt) != 0", VAS_ASSERT); } } while (0); } while (0);
...
mgt_SHM_Commit(void)
{
 ...
 do { do { if (!(snprintf(fnbuf, sizeof fnbuf, "%s.%jd", "_.vsm", (intmax_t)getpid()) < sizeof fnbuf)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 259, "snprintf(fnbuf, sizeof fnbuf, \"%s.%jd\", \"_.vsm\", (intmax_t)getpid()) < sizeof fnbuf", VAS_ASSERT); } } while (0); } while (0);
 VJ_master(JAIL_MASTER_FILE);
 if (rename(fnbuf, "_.vsm")) {
  fprintf(stderr, "Rename failed %s -> %s: %s\n",
      fnbuf, "_.vsm", strerror((*__errno_location ())));
  (void)unlink(fnbuf);
  exit(1);
 }
 VJ_master(JAIL_MASTER_LOW);
...
mgt_SHM_Destroy(int keep)
{
 ...
 do { do { if (!((heritage.vsm) != 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 281, "(heritage.vsm) != 0", VAS_ASSERT); } } while (0); } while (0);
 if (keep)
  (void)rename("_.vsm", "_.vsm" ".keep");
 heritage.panic_str = ((void *)0);
 heritage.panic_str_len = 0;
 heritage.param = ((void *)0);
 VSM_common_delete(&heritage.vsm);
 do { do { if (!((munmap(mgt_vsm_p, mgt_vsm_l)) == 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 288, "(munmap(mgt_vsm_p, mgt_vsm_l)) == 0", VAS_ASSERT); } } while (0); } while (0);
 mgt_vsm_p = ((void *)0);
 mgt_vsm_l = 0;
}
...
mgt_SHM_Size_Adjust(void)
{
 ...
 do { do { if (!((heritage.vsm) != 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 301, "(heritage.vsm) != 0", VAS_ASSERT); } } while (0); } while (0);
...
mgt_SHM_Init(void)
{
 ...
 do { do { if (!((atexit(mgt_shm_atexit)) == 0)) { VAS_Fail(__func__, "mgt/mgt_shmem.c", 341, "(atexit(mgt_shm_atexit)) == 0", VAS_ASSERT); } } while (0); } while (0);
}
