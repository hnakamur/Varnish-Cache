...
vcc_Conditional(struct vcc *tl)
{
 ...
 do { vcc__Expect(tl, '(', 64); do { if ((tl)->err) return; } while (0); vcc_NextToken(tl); } while (0);
 Fb(tl, 0, "(\n");
 do { tl->indent += 2; vcc_Expr(tl, BOOL); tl->indent -= 2; } while (0);
 do { if ((tl)->err) return; } while (0);
 Fb(tl, 1, ")\n");
 do { vcc__Expect(tl, ')', 69); do { if ((tl)->err) return; } while (0); vcc_NextToken(tl); } while (0);
}
# 83 "vcc_parse.c"
static void
vcc_IfStmt(struct vcc *tl)
{
 ...
 do { vcc__Expect(tl, 132, 87); do { if ((tl)->err) return; } while (0); vcc_NextToken(tl); } while (0);
 Fb(tl, 1, "if ");
 vcc_Conditional(tl);
 do { if ((tl)->err) return; } while (0);
 do { tl->indent += 2; vcc_Compound(tl); tl->indent -= 2; } while (0);
 do { if ((tl)->err) return; } while (0);
 while (tl->t->tok == 132) {
  if (vcc_IdIs(tl->t, "else")) {
   ...
   if (tl->t->tok != 132 || !vcc_IdIs(tl->t, "if")) {
    VSB_printf(tl->sb,
 ...
 do { Fb(tl, 1, "VRT_count(ctx, %u)%s\n", ++tl->cnt, ";"); tl->t->cnt = tl->cnt; } while (0);
}
# 142 "vcc_parse.c"
static void
vcc_Compound(struct vcc *tl)
{
 ...
 do { vcc__Expect(tl, '{', 147); do { if ((tl)->err) return; } while (0); vcc_NextToken(tl); } while (0);
 Fb(tl, 1, "{\n");
 tl->indent += 2;
 do { Fb(tl, 1, "VRT_count(ctx, %u)%s\n", ++tl->cnt, ";"); tl->t->cnt = tl->cnt; } while (0);
 while (1) {
  ...
  case 132:
   if (vcc_IdIs(tl->t, "if")) {
    ...
    do { if ((tl)->err) return; } while (0);
    if (i) {
     do { vcc__Expect(tl, ';', 187); do { if ((tl)->err) return; } while (0); vcc_NextToken(tl); } while (0);
     break;
    }
   }
...
vcc_ParseFunction(struct vcc *tl)
{
 ...
 vcc_NextToken(tl);
 do { vcc__Expect(tl, 132, 214); do { if ((tl)->err) return; } while (0);} while (0);
 if (!vcc_isCid(tl->t)) {
  VSB_printf(tl->sb,
 ...
 if (m == -2) {
  ...
  for (i = 0; method_tab[i].name != ((void *)0); i++)
   VSB_printf(tl->sb, "\t%s\n", method_tab[i].name);
  return;
 } else if (m != -1) {
  do { if (!(m < 15)) { VAS_Fail(__func__, "vcc_parse.c", 232, "m < VCL_MET_MAX", VAS_ASSERT); } } while (0);
  tl->fb = tl->fm[m];
  if (tl->mprocs[m] == ((void *)0)) {
...
vcc_ParseVcl(struct vcc *tl)
{
 ...
 do { if (!(vcc_IdIs(tl->t, "vcl"))) { VAS_Fail(__func__, "vcc_parse.c", 297, "vcc_IdIs(tl->t, \"vcl\")", VAS_ASSERT); } } while (0);
 vcc_NextToken(tl);
 tok = tl->t;
...
vcc_Parse(struct vcc *tl)
{
 ...
 if (tl->t->tok != 132 || !vcc_IdIs(tl->t, "vcl")) {
  VSB_printf(tl->sb,
 ...
 while (tl->t->tok != 131) {
  ...
  case 131:
   break;
  case 132:
   for (tp = toplev; tp->name != ((void *)0); tp++) {
