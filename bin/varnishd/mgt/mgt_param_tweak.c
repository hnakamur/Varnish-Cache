...
# 1 "<command-line>" 2
# 1 "mgt/mgt_param_tweak.c"
# 33 "mgt/mgt_param_tweak.c"
# 1 "../../config.h" 1
# 34 "mgt/mgt_param_tweak.c" 2
    ...
    const char *arg, const char *min, const char *max, const char *fmt)
{
 ...
 if (arg != ((void *)0)) {
  if (min != ((void *)0)) {
   minv = VNUM(min);
   if ((sizeof (minv) == sizeof (float) ? __isnanf (minv) : sizeof (minv) == sizeof (double) ? __isnan (minv) : __isnanl (minv))) {
  ...
  if (max != ((void *)0)) {
   maxv = VNUM(max);
   if ((sizeof (maxv) == sizeof (float) ? __isnanf (maxv) : sizeof (maxv) == sizeof (double) ? __isnan (maxv) : __isnanl (maxv))) {
  ...
  u = VNUM(arg);
  if ((sizeof (u) == sizeof (float) ? __isnanf (u) : sizeof (u) == sizeof (double) ? __isnan (u) : __isnanl (u))) {
   VSB_printf(vsb, "Not a number(%s)\n", arg);
   return (-1);
  }
  if (min != ((void *)0) && u < minv) {
   VSB_printf(vsb,
       "Must be greater or equal to %s\n", min);
   return (-1);
  }
  if (max != ((void *)0) && u > maxv) {
   VSB_printf(vsb,
    ...
    const char *min, const char *max)
{
 ...
 if (arg != ((void *)0)) {
  ...
  if (max != ((void *)0) && r > rmax) {
   VSB_printf(vsb, "Must be no more than %s\n", max);
   VSB_printf(vsb, "\n");
   return (-1);
  }
  if (min != ((void *)0) && r < rmin) {
   VSB_printf(vsb, "Must be at least %s\n", min);
   return (-1);
  }
  *dest = r;
...
tweak_vsl_buffer(struct vsb *vsb, const struct parspec *par, const char *arg)
{
 ...
 if (tweak_generic_bytes(vsb, &dest, arg, par->min, par->max))
  return (-1);
 *d1 = dest;
 do { do { if (!(snprintf(buf, sizeof buf, "%u", *d1 - 12) < sizeof buf)) { VAS_Fail(__func__, "mgt/mgt_param_tweak.c", 339, "snprintf(buf, sizeof buf, \"%u\", *d1 - 12) < sizeof buf", VAS_ASSERT); } } while (0); } while (0);
 MCF_SetMaximum("vsl_reclen", (__extension__ (__builtin_constant_p (buf) && ((size_t)(const void *)((buf) + 1) - (size_t)(const void *)(buf) == 1) ? (((const char *) (buf))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (buf) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, buf, __len); __retval; })) : __strdup (buf))));
 MCF_SetMaximum("shm_reclen", (__extension__ (__builtin_constant_p (buf) && ((size_t)(const void *)((buf) + 1) - (size_t)(const void *)(buf) == 1) ? (((const char *) (buf))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (buf) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, buf, __len); __retval; })) : __strdup (buf))));
 return (0);
}
...
tweak_vsl_reclen(struct vsb *vsb, const struct parspec *par, const char *arg)
{
 ...
 if (tweak_generic_bytes(vsb, &dest, arg, par->min, par->max))
  return (-1);
 *d1 = dest;
 do { do { if (!(snprintf(buf, sizeof buf, "%u", *d1 + 12) < sizeof buf)) { VAS_Fail(__func__, "mgt/mgt_param_tweak.c", 357, "snprintf(buf, sizeof buf, \"%u\", *d1 + 12) < sizeof buf", VAS_ASSERT); } } while (0); } while (0);
 MCF_SetMinimum("vsl_buffer", (__extension__ (__builtin_constant_p (buf) && ((size_t)(const void *)((buf) + 1) - (size_t)(const void *)(buf) == 1) ? (((const char *) (buf))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (buf) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, buf, __len); __retval; })) : __strdup (buf))));
 return (0);
}
...
{
 char **p = ((void*)(uintptr_t)(par->priv));

 do { do { if (!((p) != 0)) { VAS_Fail(__func__, "mgt/mgt_param_tweak.c", 369, "(p) != 0", VAS_ASSERT); } } while (0); } while (0);

 if (arg == ((void *)0)) {
  VSB_quote(vsb, *p, -1, 0);
 } else {
  do { free(*p); if ((arg) != ((void *)0)) { *p = (__extension__ (__builtin_constant_p (arg) && ((size_t)(const void *)((arg) + 1) - (size_t)(const void *)(arg) == 1) ? (((const char *) (arg))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (arg) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, arg, __len); __retval; })) : __strdup (arg))); do { do { if (!(((*p)) != 0)) { VAS_Fail(__func__, "mgt/mgt_param_tweak.c", 374, "((*p)) != 0", VAS_ASSERT); } } while (0); } while (0); } else { *p = ((void *)0); } } while (0);
 }
 return (0);
}
