...
VAV_BackSlash(const char *s, char *res)
{
 ...
 do { if (!(*s == '\\')) { VAS_Fail(__func__, "../libvarnish/vav.c", 56, "*s == '\\\\'", VAS_ASSERT); } } while (0);
 r = c = 0;
 switch(s[1]) {
 case 'n':
  c = '\n';
 ...
 case '4': case '5': case '6': case '7':
  for (r = 1; r < 4; r++) {
   if (!((*__ctype_b_loc ())[(int) ((s[r]))] & (unsigned short int) _ISdigit))
    break;
   if (s[r] - '0' > 7)
    break;
   c <<= 3;
 ...
 case 'x':
  if (1 == sscanf(s + 1, "x%02x", &u)) {
   do { do { if (!((u & ~0xff) == 0)) { VAS_Fail(__func__, "../libvarnish/vav.c", 92, "(u & ~0xff) == 0", VAS_ASSERT); } } while (0); } while (0);
   c = u;
   r = 4;
  }
  break;
...
VAV_BackSlashDecode(const char *s, const char *e)
{
 ...
 if (e == ((void *)0))
  e = (__extension__ (__builtin_constant_p ('\0') && !__builtin_constant_p (s) && ('\0') == '\0' ? (char *) __rawmemchr (s, '\0') : __builtin_strchr (s, '\0')));
 do { if (!(e != ((void *)0))) { VAS_Fail(__func__, "../libvarnish/vav.c", 114, "e != NULL", VAS_ASSERT); } } while (0);
 p = calloc((e - s) + 1, 1);
 if (p == ((void *)0))
  return (p);
 for (r = p, q = s; q < e; ) {
...
VAV_Parse(const char *s, int *argc, int flag)
{
 ...
 do { if (!(s != ((void *)0))) { VAS_Fail(__func__, "../libvarnish/vav.c", 142, "s != NULL", VAS_ASSERT); } } while (0);
 nargv = 1;
 largv = 16;
 ...
 for (;;) {
  ...
  while (1) {
   ...
   if (!quote) {
    if (*s == '\0' || ((*__ctype_b_loc ())[(int) ((*s))] & (unsigned short int) _ISspace))
     break;
    if ((flag & (1 << 1)) && *s == ',')
     break;
    s++;
  ...
  if (nargv + 1 >= largv) {
   ...
   do { if (!(argv != ((void *)0))) { VAS_Fail(__func__, "../libvarnish/vav.c", 193, "argv != NULL", VAS_ASSERT); } } while (0);
  }
  if (flag & (1 << 2)) {
   argv[nargv] = malloc(1 + (s - p));
   do { if (!(argv[nargv] != ((void *)0))) { VAS_Fail(__func__, "../libvarnish/vav.c", 197, "argv[nargv] != NULL", VAS_ASSERT); } } while (0);
