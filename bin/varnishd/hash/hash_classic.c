...
# 1 "<command-line>" 2
# 1 "hash/hash_classic.c"
# 32 "hash/hash_classic.c"
# 1 "../../config.h" 1
# 33 "hash/hash_classic.c" 2
...
struct hcl_hd {
 ...
 struct { struct objhead *vtqh_first; struct objhead **vtqh_last; } head;
 struct lock mtx;
};
...
hcl_init(int ac, char * const *av)
{
 ...
 if (ac == 0)
  return;
 if (ac > 1)
  do { fprintf(stderr, "Error: " "(-hclassic) too many arguments\n"); exit(2); } while (0);
 i = sscanf(av[0], "%u", &u);
 if (i <= 0 || u == 0)
  return;
 if (u > 2 && !(u & (u - 1))) {
...
hcl_start(void)
{
 ...
 do { do { if (!((hcl_head) != 0)) { VAS_Fail(__func__, "hash/hash_classic.c", 97, "(hcl_head) != 0", VAS_MISSING); } } while (0); } while (0);

 for (u = 0; u < hcl_nhash; u++) {
  do { (((&hcl_head[u].head))->vtqh_first) = ((void *)0); (&hcl_head[u].head)->vtqh_last = &(((&hcl_head[u].head))->vtqh_first); } while (0);
  Lck__New(&hcl_head[u].mtx, lck_hcl, "lck_hcl");
  hcl_head[u].magic = 0x0f327016;
 }
}
# 115 "hash/hash_classic.c"
...
hcl_lookup(struct worker *wrk, const void *digest, struct objhead **noh)
{
 ...
 do { do { if (!((wrk) != ((void *)0))) { VAS_Fail(__func__, "hash/hash_classic.c", 123, "(wrk) != NULL", VAS_ASSERT); } } while (0); do { if (!((wrk)->magic == 0x6391adcf)) { VAS_Fail(__func__, "hash/hash_classic.c", 123, "(wrk)->magic == 0x6391adcf", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((digest) != 0)) { VAS_Fail(__func__, "hash/hash_classic.c", 124, "(digest) != 0", VAS_ASSERT); } } while (0); } while (0);
 if (noh != ((void *)0))
  do { do { if (!((*noh) != ((void *)0))) { VAS_Fail(__func__, "hash/hash_classic.c", 126, "(*noh) != NULL", VAS_ASSERT); } } while (0); do { if (!((*noh)->magic == 0x1b96615d)) { VAS_Fail(__func__, "hash/hash_classic.c", 126, "(*noh)->magic == 0x1b96615d", VAS_ASSERT); } } while (0); } while (0);

 do { if (!(sizeof oh->digest >= sizeof hdigest)) { VAS_Fail(__func__, "hash/hash_classic.c", 128, "sizeof oh->digest >= sizeof hdigest", VAS_ASSERT); } } while (0);
 memcpy(&hdigest, digest, sizeof hdigest);
 u1 = hdigest % hcl_nhash;
 hp = &hcl_head[u1];

 Lck__Lock(&hp->mtx, __func__, 133);
 for ((oh) = (((&hp->head))->vtqh_first); (oh); (oh) = (((oh))->_u.n.u_n_hoh_list.vtqe_next)) {
  do { do { if (!((oh) != ((void *)0))) { VAS_Fail(__func__, "hash/hash_classic.c", 135, "(oh) != NULL", VAS_ASSERT); } } while (0); do { if (!((oh)->magic == 0x1b96615d)) { VAS_Fail(__func__, "hash/hash_classic.c", 135, "(oh)->magic == 0x1b96615d", VAS_ASSERT); } } while (0); } while (0);
  ...
  if (i > 0)
   break;
  oh->refcnt++;
  Lck__Unlock(&hp->mtx, __func__, 142);
  Lck__Lock(&oh->mtx, __func__, 143);
  return (oh);
 }

 if (noh == ((void *)0)) {
  Lck__Unlock(&hp->mtx, __func__, 148);
  return (((void *)0));
 }

 if (oh != ((void *)0))
  do { (*noh)->_u.n.u_n_hoh_list.vtqe_prev = (oh)->_u.n.u_n_hoh_list.vtqe_prev; (((*noh))->_u.n.u_n_hoh_list.vtqe_next) = (oh); *(oh)->_u.n.u_n_hoh_list.vtqe_prev = (*noh); (oh)->_u.n.u_n_hoh_list.vtqe_prev = &(((*noh))->_u.n.u_n_hoh_list.vtqe_next); } while (0);
 else
  do { (((*noh))->_u.n.u_n_hoh_list.vtqe_next) = ((void *)0); (*noh)->_u.n.u_n_hoh_list.vtqe_prev = (&hp->head)->vtqh_last; *(&hp->head)->vtqh_last = (*noh); (&hp->head)->vtqh_last = &(((*noh))->_u.n.u_n_hoh_list.vtqe_next); } while (0);

 oh = *noh;
 *noh = ((void *)0);
 memcpy(oh->digest, digest, sizeof oh->digest);

 oh->_u.n.u_n_hoh_head = hp;

 Lck__Unlock(&hp->mtx, __func__, 163);
 Lck__Lock(&oh->mtx, __func__, 164);
 return (oh);
}
...
hcl_deref(struct objhead *oh)
{
 ...
 do { do { if (!((oh) != ((void *)0))) { VAS_Fail(__func__, "hash/hash_classic.c", 178, "(oh) != NULL", VAS_ASSERT); } } while (0); do { if (!((oh)->magic == 0x1b96615d)) { VAS_Fail(__func__, "hash/hash_classic.c", 178, "(oh)->magic == 0x1b96615d", VAS_ASSERT); } } while (0); } while (0);
 do { (hp) = (oh->_u.n.u_n_hoh_head); do { if (!((hp) != ((void *)0))) { VAS_Fail(__func__, "hash/hash_classic.c", 179, "(hp) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((hp))->magic == (0x0f327016))) { VAS_Fail(__func__, "hash/hash_classic.c", 179, "((hp))->magic == (0x0f327016)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 do { if (!(oh->refcnt > 0)) { VAS_Fail(__func__, "hash/hash_classic.c", 180, "oh->refcnt > 0", VAS_ASSERT); } } while (0);
 Lck__Lock(&hp->mtx, __func__, 181);
 if (--oh->refcnt == 0) {
  do { if (((((oh))->_u.n.u_n_hoh_list.vtqe_next)) != ((void *)0)) (((oh))->_u.n.u_n_hoh_list.vtqe_next)->_u.n.u_n_hoh_list.vtqe_prev = (oh)->_u.n.u_n_hoh_list.vtqe_prev; else { (&hp->head)->vtqh_last = (oh)->_u.n.u_n_hoh_list.vtqe_prev; } *(oh)->_u.n.u_n_hoh_list.vtqe_prev = (((oh))->_u.n.u_n_hoh_list.vtqe_next); ; ; } while (0);
  ret = 0;
 } else
  ret = 1;
 Lck__Unlock(&hp->mtx, __func__, 187);
 return (ret);
}
