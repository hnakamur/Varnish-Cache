...
# 1 "<command-line>" 2
# 1 "cache/cache_wrk.c"
# 32 "cache/cache_wrk.c"
# 1 "../../config.h" 1
# 33 "cache/cache_wrk.c" 2
...
wrk_bgthread(void *arg)
{
 ...
 do { (bt) = (arg); do { if (!((bt) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 65, "(bt) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((bt))->magic == (0x23b5152b))) { VAS_Fail(__func__, "cache/cache_wrk.c", 65, "((bt))->magic == (0x23b5152b)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 THR_SetName(bt->name);
 do { (void)memset(&wrk, 0, sizeof *&wrk); (&wrk)->magic = (0x6391adcf); } while (0);

 (void)bt->func(&wrk, bt->priv);

 do { VAS_Fail(__func__, "cache/cache_wrk.c", 71, "BgThread terminated", VAS_WRONG); } while (0);
...
WRK_BgThread(pthread_t *thr, const char *name, bgthread_t *func, void *priv)
{
 ...
 do { (bt) = calloc(sizeof *(bt), 1); if ((bt) != ((void *)0)) (bt)->magic = (0x23b5152b); } while (0);
 do { do { if (!((bt) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 82, "(bt) != 0", VAS_ASSERT); } } while (0); } while (0);

 bt->name = name;
 bt->func = func;
 bt->priv = priv;
 do { do { if (!((pthread_create(thr, ((void *)0), wrk_bgthread, bt)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 87, "(pthread_create(thr, ((void *)0), wrk_bgthread, bt)) == 0", VAS_ASSERT); } } while (0); } while (0);
}
...
WRK_Thread(struct pool *qp, size_t stacksize, unsigned thread_workspace)
{
 ...
 do { do { if (!((qp) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 99, "(qp) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((stacksize) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 100, "(stacksize) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((thread_workspace) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 101, "(thread_workspace) != 0", VAS_ASSERT); } } while (0); } while (0);

 THR_SetName("cache-worker");
 w = &ww;
 do { (void)memset(w, 0, sizeof *w); (w)->magic = (0x6391adcf); } while (0);
 w->lastused = (__builtin_nanf (""));
 do { do { if (!((pthread_cond_init(&w->cond, ((void *)0))) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 107, "(pthread_cond_init(&w->cond, ((void *)0))) == 0", VAS_ASSERT); } } while (0); } while (0);

 WS_Init(w->aws, "wrk", ws, thread_workspace);

 u = getpagesize();
 do { do { if (!((u) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 112, "(u) != 0", VAS_ASSERT); } } while (0); } while (0);
 u -= 1U;
 ...
 do { do { if (!((w->pool) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 122, "(w->pool) == 0", VAS_ASSERT); } } while (0); } while (0);

 VSL(SLT_WorkThread, 0, "%p end", w);
 if (w->vcl != ((void *)0))
  VCL_Rel(&w->vcl);
 do { do { if (!((pthread_cond_destroy(&w->cond)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 127, "(pthread_cond_destroy(&w->cond)) == 0", VAS_ASSERT); } } while (0); } while (0);
 HSH_Cleanup(w);
 Pool_Sumstat(w);
}
...
;
# 48 "../../include/tbl/vsc_f_main.h"
(dst->sess_conn += src->sess_conn);




(dst->sess_drop += src->sess_drop);




(dst->sess_fail += src->sess_fail);
# 67 "../../include/tbl/vsc_f_main.h"
(dst->client_req_400 += src->client_req_400);





(dst->client_req_417 += src->client_req_417);




(dst->client_req += src->client_req);
...
(dst->cache_hitpass += src->cache_hitpass);
# 101 "../../include/tbl/vsc_f_main.h"
...
;
# 150 "../../include/tbl/vsc_f_main.h"
(dst->fetch_head += src->fetch_head);



(dst->fetch_length += src->fetch_length);



(dst->fetch_chunked += src->fetch_chunked);



(dst->fetch_eof += src->fetch_eof);



(dst->fetch_bad += src->fetch_bad);



(dst->fetch_none += src->fetch_none);



(dst->fetch_1xx += src->fetch_1xx);



(dst->fetch_204 += src->fetch_204);



(dst->fetch_304 += src->fetch_304);
...
(dst->fetch_no_thread += src->fetch_no_thread);
# 201 "../../include/tbl/vsc_f_main.h"
;
...
(dst->busy_sleep += src->busy_sleep);





(dst->busy_wakeup += src->busy_wakeup);





(dst->busy_killed += src->busy_killed);
...
(dst->n_object += src->n_object);



(dst->n_vampireobject += src->n_vampireobject);



(dst->n_objectcore += src->n_objectcore);



(dst->n_objecthead += src->n_objecthead);



(dst->n_waitinglist += src->n_waitinglist);
...
(dst->s_pipe += src->s_pipe);



(dst->s_pass += src->s_pass);



(dst->s_fetch += src->s_fetch);



(dst->s_synth += src->s_synth);



(dst->s_req_hdrbytes += src->s_req_hdrbytes);



(dst->s_req_bodybytes += src->s_req_bodybytes);



(dst->s_resp_hdrbytes += src->s_resp_hdrbytes);



(dst->s_resp_bodybytes += src->s_resp_bodybytes);
...
(dst->sess_closed += src->sess_closed);
...
(dst->sess_readahead += src->sess_readahead);



(dst->sess_herd += src->sess_herd);
# 406 "../../include/tbl/vsc_f_main.h"
# 1 "../../include/tbl/sess_close.h" 1
# 33 "../../include/tbl/sess_close.h"
;
...
pool_getidleworker(struct pool *pp)
{
 ...
 do { do { if (!((pp) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 159, "(pp) != NULL", VAS_ASSERT); } } while (0); do { if (!((pp)->magic == 0x606658fa)) { VAS_Fail(__func__, "cache/cache_wrk.c", 159, "(pp)->magic == 0x606658fa", VAS_ASSERT); } } while (0); } while (0);
 Lck__Assert(&pp->mtx, 1);
 pt = ((&pp->idle_queue)->vtqh_first);
 if (pt == ((void *)0)) {
  if (pp->nthr < cache_param->wthread_max) {
   pp->dry++;
   do { do { if (!((pthread_cond_signal(&pp->herder_cond)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 165, "(pthread_cond_signal(&pp->herder_cond)) == 0", VAS_ASSERT); } } while (0); } while (0);
  }
  return (((void *)0));
 }
 do { do { if (!((pt->func) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 169, "(pt->func) == 0", VAS_ASSERT); } } while (0); } while (0);
 do { (wrk) = (pt->priv); do { if (!((wrk) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 170, "(wrk) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((wrk))->magic == (0x6391adcf))) { VAS_Fail(__func__, "cache/cache_wrk.c", 170, "((wrk))->magic == (0x6391adcf)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 return (wrk);
}
# 181 "cache/cache_wrk.c"
int
    ...
    const void *arg, size_t arg_len)
{
 ...
 do { do { if (!((wrk) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 189, "(wrk) != NULL", VAS_ASSERT); } } while (0); do { if (!((wrk)->magic == 0x6391adcf)) { VAS_Fail(__func__, "cache/cache_wrk.c", 189, "(wrk)->magic == 0x6391adcf", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((arg) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 190, "(arg) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((arg_len) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 191, "(arg_len) != 0", VAS_ASSERT); } } while (0); } while (0);
 pp = wrk->pool;
 do { do { if (!((pp) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 193, "(pp) != NULL", VAS_ASSERT); } } while (0); do { if (!((pp)->magic == 0x606658fa)) { VAS_Fail(__func__, "cache/cache_wrk.c", 193, "(pp)->magic == 0x606658fa", VAS_ASSERT); } } while (0); } while (0);

 Lck__Lock(&pp->mtx, __func__, 195);
 wrk2 = pool_getidleworker(pp);
 if (wrk2 != ((void *)0)) {
  do { if (((((&wrk2->task))->list.vtqe_next)) != ((void *)0)) (((&wrk2->task))->list.vtqe_next)->list.vtqe_prev = (&wrk2->task)->list.vtqe_prev; else { (&pp->idle_queue)->vtqh_last = (&wrk2->task)->list.vtqe_prev; } *(&wrk2->task)->list.vtqe_prev = (((&wrk2->task))->list.vtqe_next); ; ; } while (0);
  retval = 1;
 } else {
  wrk2 = wrk;
  retval = 0;
 }
 Lck__Unlock(&pp->mtx, __func__, 204);
 do { do { if (!((wrk2->task.func) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 205, "(wrk2->task.func) == 0", VAS_ASSERT); } } while (0); } while (0);

 do { if (!(arg_len <= WS_Reserve(wrk2->aws, arg_len))) { VAS_Fail(__func__, "cache/cache_wrk.c", 207, "arg_len <= WS_Reserve(wrk2->aws, arg_len)", VAS_ASSERT); } } while (0);
 memcpy(wrk2->aws->f, arg, arg_len);
 wrk2->task.func = func;
 wrk2->task.priv = wrk2->aws->f;
 if (retval)
  do { do { if (!((pthread_cond_signal(&wrk2->cond)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 212, "(pthread_cond_signal(&wrk2->cond)) == 0", VAS_ASSERT); } } while (0); } while (0);
 return (retval);
}
...
Pool_Task(struct pool *pp, struct pool_task *task, enum task_how how)
{
 ...
 do { do { if (!((pp) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 226, "(pp) != NULL", VAS_ASSERT); } } while (0); do { if (!((pp)->magic == 0x606658fa)) { VAS_Fail(__func__, "cache/cache_wrk.c", 226, "(pp)->magic == 0x606658fa", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((task) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 227, "(task) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((task->func) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 228, "(task->func) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { if (!(how < TASK_QUEUE_END)) { VAS_Fail(__func__, "cache/cache_wrk.c", 229, "how < TASK_QUEUE_END", VAS_ASSERT); } } while (0);

 Lck__Lock(&pp->mtx, __func__, 231);



 wrk = pool_getidleworker(pp);
 if (wrk != ((void *)0)) {
  do { if (((((&wrk->task))->list.vtqe_next)) != ((void *)0)) (((&wrk->task))->list.vtqe_next)->list.vtqe_prev = (&wrk->task)->list.vtqe_prev; else { (&pp->idle_queue)->vtqh_last = (&wrk->task)->list.vtqe_prev; } *(&wrk->task)->list.vtqe_prev = (((&wrk->task))->list.vtqe_next); ; ; } while (0);
  do { do { if (!((wrk->task.func) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 238, "(wrk->task.func) == 0", VAS_ASSERT); } } while (0); } while (0);
  wrk->task.func = task->func;
  wrk->task.priv = task->priv;
  Lck__Unlock(&pp->mtx, __func__, 241);
  do { do { if (!((pthread_cond_signal(&wrk->cond)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 242, "(pthread_cond_signal(&wrk->cond)) == 0", VAS_ASSERT); } } while (0); } while (0);
  return (0);
 }
 ...
 if (how == TASK_QUEUE_VCA ||
  ...
  pp->nqueued++;
  pp->lqueue++;
  do { (((task))->list.vtqe_next) = ((void *)0); (task)->list.vtqe_prev = (&pp->queues[how])->vtqh_last; *(&pp->queues[how])->vtqh_last = (task); (&pp->queues[how])->vtqh_last = &(((task))->list.vtqe_next); } while (0);
 } else {
  pp->ndropped++;
  retval = -1;
 }
 Lck__Unlock(&pp->mtx, __func__, 257);
 return (retval);
}
...
Pool_Work_Thread(struct pool *pp, struct worker *wrk)
{
 ...
 do { do { if (!((pp) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 284, "(pp) != NULL", VAS_ASSERT); } } while (0); do { if (!((pp)->magic == 0x606658fa)) { VAS_Fail(__func__, "cache/cache_wrk.c", 284, "(pp)->magic == 0x606658fa", VAS_ASSERT); } } while (0); } while (0);
 wrk->pool = pp;
 while (1) {
  Lck__Lock(&pp->mtx, __func__, 287);

  do { do { if (!((wrk) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 289, "(wrk) != NULL", VAS_ASSERT); } } while (0); do { if (!((wrk)->magic == 0x6391adcf)) { VAS_Fail(__func__, "cache/cache_wrk.c", 289, "(wrk)->magic == 0x6391adcf", VAS_ASSERT); } } while (0); } while (0);

  WS_Reset(wrk->aws, ((void *)0));
  do { do { if (!((wrk->vsl) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 292, "(wrk->vsl) == 0", VAS_ASSERT); } } while (0); } while (0);

  for (i = 0; i < TASK_QUEUE_END; i++) {
   tp = ((&pp->queues[i])->vtqh_first);
   if (tp != ((void *)0)) {
    pp->lqueue--;
    do { if (((((tp))->list.vtqe_next)) != ((void *)0)) (((tp))->list.vtqe_next)->list.vtqe_prev = (tp)->list.vtqe_prev; else { (&pp->queues[i])->vtqh_last = (tp)->list.vtqe_prev; } *(tp)->list.vtqe_prev = (((tp))->list.vtqe_next); ; ; } while (0);
    break;
   }
  }

  if ((tp == ((void *)0) && wrk->stats->summs > 0) ||
      (wrk->stats->summs >= cache_param->wthread_stats_rate))
   pool_addstat(pp->a_stat, wrk->stats);

  if (tp != ((void *)0)) {
   wrk->stats->summs++;
  } else if (pp->b_stat != ((void *)0) && pp->a_stat->summs) {
   ...
   if ((sizeof (wrk->lastused) == sizeof (float) ? __isnanf (wrk->lastused) : sizeof (wrk->lastused) == sizeof (double) ? __isnan (wrk->lastused) : __isnanl (wrk->lastused)))
    wrk->lastused = VTIM_real();
   wrk->task.func = ((void *)0);
   wrk->task.priv = wrk;
   do { if (((((&wrk->task))->list.vtqe_next) = (((&pp->idle_queue))->vtqh_first)) != ((void *)0)) (((&pp->idle_queue))->vtqh_first)->list.vtqe_prev = &(((&wrk->task))->list.vtqe_next); else (&pp->idle_queue)->vtqh_last = &(((&wrk->task))->list.vtqe_next); (((&pp->idle_queue))->vtqh_first) = (&wrk->task); (&wrk->task)->list.vtqe_prev = &(((&pp->idle_queue))->vtqh_first); } while (0);
   do {
    i = Lck_CondWait(&wrk->cond, &pp->mtx,
        wrk->vcl == ((void *)0) ? 0 : wrk->lastused+60.);
    if (i == 110)
     VCL_Rel(&wrk->vcl);
   } while (wrk->task.func == ((void *)0));
   tpx = wrk->task;
   tp = &tpx;
   wrk->stats->summs++;
  }
  Lck__Unlock(&pp->mtx, __func__, 333);
  ...
  do {
   ...
   do { if (!(wrk->pool == pp)) { VAS_Fail(__func__, "cache/cache_wrk.c", 340, "wrk->pool == pp", VAS_ASSERT); } } while (0);
   tp->func(wrk, tp->priv);
   tpx = wrk->task;
   tp = &tpx;
  } while (tp->func != ((void *)0));
...
pool_thread(void *priv)
{
 ...
 do { (pi) = (priv); do { if (!((pi) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 368, "(pi) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((pi))->magic == (0x4e4442d3))) { VAS_Fail(__func__, "cache/cache_wrk.c", 368, "((pi))->magic == (0x4e4442d3)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 WRK_Thread(pi->qp, pi->stacksize, cache_param->workspace_thread);
 do { (pi)->magic = (0); free(pi); pi = ((void *)0); } while (0);
 return (((void *)0));
}
...
pool_breed(struct pool *qp)
{
 ...
 do { do { if (!((pthread_attr_init(&tp_attr)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 381, "(pthread_attr_init(&tp_attr)) == 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((pthread_attr_setdetachstate(&tp_attr, PTHREAD_CREATE_DETACHED)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 382, "(pthread_attr_setdetachstate(&tp_attr, PTHREAD_CREATE_DETACHED)) == 0", VAS_ASSERT); } } while (0); } while (0);


 if (cache_param->wthread_stacksize != (2147483647 * 2U + 1U))
  do { do { if (!((pthread_attr_setstacksize(&tp_attr, cache_param->wthread_stacksize)) == 0)) { VAS_Fail(__func__,
 "cache/cache_wrk.c"
# 386 "cache/cache_wrk.c"
  ,
 387
# 386 "cache/cache_wrk.c"
  , "(pthread_attr_setstacksize(&tp_attr, cache_param->wthread_stacksize)) == 0", VAS_ASSERT); } } while (0); } while (0)
                                      ;

 do { (pi) = calloc(sizeof *(pi), 1); if ((pi) != ((void *)0)) (pi)->magic = (0x4e4442d3); } while (0);
 do { do { if (!((pi) != 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 390, "(pi) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((pthread_attr_getstacksize(&tp_attr, &pi->stacksize)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 391, "(pthread_attr_getstacksize(&tp_attr, &pi->stacksize)) == 0", VAS_ASSERT); } } while (0); } while (0);
 pi->qp = qp;

 if (pthread_create(&tp, &tp_attr, pool_thread, pi)) {
  VSL(SLT_Debug, 0, "Create worker thread failed %d %s",
      (*__errno_location ()), strerror((*__errno_location ())));
  Lck__Lock(&pool_mtx, __func__, 397);
  VSC_C_main->threads_failed++;
  Lck__Unlock(&pool_mtx, __func__, 399);
  VTIM_sleep(cache_param->wthread_fail_delay);
 } else {
  qp->dry = 0;
  qp->nthr++;
  Lck__Lock(&pool_mtx, __func__, 404);
  VSC_C_main->threads++;
  VSC_C_main->threads_created++;
  Lck__Unlock(&pool_mtx, __func__, 407);
  VTIM_sleep(cache_param->wthread_add_delay);
 }

 do { do { if (!((pthread_attr_destroy(&tp_attr)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 411, "(pthread_attr_destroy(&tp_attr)) == 0", VAS_ASSERT); } } while (0); } while (0);
}
# 429 "cache/cache_wrk.c"
void*
pool_herder(void *priv)
{
 ...
 do { (pp) = (priv); do { if (!((pp) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 437, "(pp) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((pp))->magic == (0x606658fa))) { VAS_Fail(__func__, "cache/cache_wrk.c", 437, "((pp))->magic == (0x606658fa)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 ...
 while (1) {
  ...
  do { if (!(pp->nthr >= cache_param->wthread_min)) { VAS_Fail(__func__, "cache/cache_wrk.c", 446, "pp->nthr >= cache_param->wthread_min", VAS_ASSERT); } } while (0);

  if (pp->nthr > cache_param->wthread_min) {

   t_idle = VTIM_real() - cache_param->wthread_timeout;

   Lck__Lock(&pp->mtx, __func__, 452);
   ...
   wrk = ((void *)0);
   pt = (*(((struct taskhead *)((&pp->idle_queue)->vtqh_last))->vtqh_last));
   if (pt != ((void *)0)) {
    do { do { if (!((pt->func) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 461, "(pt->func) == 0", VAS_ASSERT); } } while (0); } while (0);
    do { (wrk) = (pt->priv); do { if (!((wrk) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_wrk.c", 462, "(wrk) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((wrk))->magic == (0x6391adcf))) { VAS_Fail(__func__, "cache/cache_wrk.c", 462, "((wrk))->magic == (0x6391adcf)", VAS_ASSERT); } } while (0); } while (0); } while (0);
    ...
    if (wrk->lastused < t_idle ||
     ...
     do { do { if (!((pthread_cond_signal(&wrk->cond)) == 0)) { VAS_Fail(__func__, "cache/cache_wrk.c", 470, "(pthread_cond_signal(&wrk->cond)) == 0", VAS_ASSERT); } } while (0); } while (0);
    } else
     wrk = ((void *)0);
   }
   Lck__Unlock(&pp->mtx, __func__, 474);

   if (wrk != ((void *)0)) {
    pp->nthr--;
    Lck__Lock(&pool_mtx, __func__, 478);
    VSC_C_main->threads--;
    VSC_C_main->threads_destroyed++;
    Lck__Unlock(&pool_mtx, __func__, 481);
    VTIM_sleep(cache_param->wthread_destroy_delay);
    continue;
   }
  }

  Lck__Lock(&pp->mtx, __func__, 487);
  if (!pp->dry) {
  ...
  Lck__Unlock(&pp->mtx, __func__, 496);
 }
 return (((void *)0));
}
