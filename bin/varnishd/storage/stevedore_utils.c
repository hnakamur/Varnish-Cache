...
# 1 "<command-line>" 2
# 1 "storage/stevedore_utils.c"
# 32 "storage/stevedore_utils.c"
# 1 "../../config.h" 1
# 33 "storage/stevedore_utils.c" 2
...
# 35 "storage/stevedore_utils.c" 2
...
# 36 "storage/stevedore_utils.c" 2
...
# 38 "storage/stevedore_utils.c" 2
# 1 "/usr/include/sys/mount.h" 1 3 4
# 25 "/usr/include/sys/mount.h" 3 4
...
# 26 "/usr/include/sys/mount.h" 2 3 4
...
# 39 "storage/stevedore_utils.c" 2
...
# 42 "storage/stevedore_utils.c" 2
...
# 45 "storage/stevedore_utils.c" 2
...
typedef int error_t;
# 48 "storage/stevedore_utils.c" 2
...
# 49 "storage/stevedore_utils.c" 2
...
# 50 "storage/stevedore_utils.c" 2
...
# 51 "storage/stevedore_utils.c" 2
...
# 52 "storage/stevedore_utils.c" 2
...
# 53 "storage/stevedore_utils.c" 2
...
# 55 "storage/stevedore_utils.c" 2
...
# 57 "storage/stevedore_utils.c" 2
# 1 "../../include/vnum.h" 1
# 32 "../../include/vnum.h"
...
# 58 "storage/stevedore_utils.c" 2
# 1 "../../include/vfil.h" 1
# 32 "../../include/vfil.h"
...
# 59 "storage/stevedore_utils.c" 2
# 80 "storage/stevedore_utils.c"
int
STV_GetFile(const char *fn, int *fdp, const char **fnp, const char *ctx)
{
 ...
 do { do { if (!((fn) != 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 88, "(fn) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((fnp) != 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 89, "(fnp) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((fdp) != 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 90, "(fdp) != 0", VAS_ASSERT); } } while (0); } while (0);
 *fnp = ((void *)0);
 *fdp = -1;


 VJ_master(JAIL_MASTER_STORAGE);
 fd = open(fn, 02 | 0100 | 0200 | 0, 0600);
 if (fd >= 0) {
 ...
 if (stat(fn, &st))
  do { fprintf(stderr, "Error: " "(%s) \"%s\" does not exist and could not be created\n", ctx, fn); exit(2); } while (0)

              ;

 if (((((st.st_mode)) & 0170000) == (0040000))) {
  do { do { if (!(snprintf(buf, sizeof buf, "%s/varnish.XXXXXX", fn) < sizeof buf)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 111, "snprintf(buf, sizeof buf, \"%s/varnish.XXXXXX\", fn) < sizeof buf", VAS_ASSERT); } } while (0); } while (0);
  fd = mkstemp(buf);
  if (fd < 0)
   do { fprintf(stderr, "Error: " "(%s) \"%s\" mkstemp(%s) failed (%s)\n", ctx, fn, buf, strerror((*__errno_location ()))); exit(2); } while (0)
                                     ;
  do { do { if (!((unlink(buf)) == 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 116, "(unlink(buf)) == 0", VAS_ASSERT); } } while (0); } while (0);
  *fnp = (__extension__ (__builtin_constant_p (buf) && ((size_t)(const void *)((buf) + 1) - (size_t)(const void *)(buf) == 1) ? (((const char *) (buf))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (buf) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, buf, __len); __retval; })) : __strdup (buf)));
  do { do { if (!((*fnp) != 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 118, "(*fnp) != 0", VAS_ASSERT); } } while (0); } while (0);
  retval = 2;
 } else if (((((st.st_mode)) & 0170000) == (0100000))) {
  fd = open(fn, 02 | 0);
  if (fd < 0)
   do { fprintf(stderr, "Error: " "(%s) \"%s\" could not open (%s)\n", ctx, fn, strerror((*__errno_location ()))); exit(2); } while (0)
                                ;
  *fnp = fn;
  retval = 0;
 } else
  do { fprintf(stderr, "Error: " "(%s) \"%s\" is neither file nor directory\n", ctx, fn); exit(2); } while (0)
                                                             ;

 do { do { if (!((fstat(fd, &st)) == 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 131, "(fstat(fd, &st)) == 0", VAS_ASSERT); } } while (0); } while (0);
 if (!((((st.st_mode)) & 0170000) == (0100000)))
  do { fprintf(stderr, "Error: " "(%s) \"%s\" was not a file after opening\n", ctx, fn); exit(2); } while (0)
              ;
...
STV_FileSize(int fd, const char *size, unsigned *granularity, const char *ctx)
{
 ...
 do { do { if (!((granularity) != 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 161, "(granularity) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((ctx) != 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 162, "(ctx) != 0", VAS_ASSERT); } } while (0); } while (0);

 do { do { if (!((fstat(fd, &st)) == 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 164, "(fstat(fd, &st)) == 0", VAS_ASSERT); } } while (0); } while (0);
 do { if (!(((((st.st_mode)) & 0170000) == (0100000)))) { VAS_Fail(__func__, "storage/stevedore_utils.c", 165, "S_ISREG(st.st_mode)", VAS_MISSING); } } while (0);

 do { do { if (!((VFIL_fsinfo(fd, &bs, &fssize, ((void *)0))) == 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 167, "(VFIL_fsinfo(fd, &bs, &fssize, ((void *)0))) == 0", VAS_ASSERT); } } while (0); } while (0);

 if (*granularity < bs)
  *granularity = bs;

 if ((size == ((void *)0) || *size == '\0') && st.st_size != 0) {




  l = st.st_size;
 } else if (size == ((void *)0) || *size == '\0') {
  do { fprintf(stderr, "Error: " "(%s) no size specified\n", ctx); exit(2); } while (0)
          ;
 } else {
  do { do { if (!((size) != 0)) { VAS_Fail(__func__, "storage/stevedore_utils.c", 182, "(size) != 0", VAS_ASSERT); } } while (0); } while (0);
  q = VNUM_2bytes(size, &l, 0);

  if (q != ((void *)0))
   do { fprintf(stderr, "Error: " "(%s) size \"%s\": %s\n", ctx, size, q); exit(2); } while (0);

  if (l < 1024*1024)
   do { fprintf(stderr, "Error: " "(%s) size \"%s\": too small, " "did you forget to specify M or G?\n", ctx, size); exit(2); } while (0)
                                                        ;

  if (l > fssize)
   do { fprintf(stderr, "Error: " "(%s) size \"%s\": larger than file system\n", ctx, size); exit(2); } while (0)
                 ;
 }
