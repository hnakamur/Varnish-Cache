...
# 1 "<command-line>" 2
# 1 "cache/cache_pool.c"
# 36 "cache/cache_pool.c"
# 1 "../../config.h" 1
# 37 "cache/cache_pool.c" 2
...
# 43 "cache/cache_pool.c" 2

static pthread_t thr_pool_herder;

static struct lock wstat_mtx;
struct lock pool_mtx;
static struct { struct pool *vtqh_first; struct pool **vtqh_last; } pools = { ((void *)0), &(pools).vtqh_first, };
...
;
# 48 "../../include/tbl/vsc_f_main.h"
(VSC_C_main->sess_conn += src->sess_conn);




(VSC_C_main->sess_drop += src->sess_drop);




(VSC_C_main->sess_fail += src->sess_fail);
# 67 "../../include/tbl/vsc_f_main.h"
(VSC_C_main->client_req_400 += src->client_req_400);





(VSC_C_main->client_req_417 += src->client_req_417);




(VSC_C_main->client_req += src->client_req);
...
(VSC_C_main->cache_hit += src->cache_hit);
...
(VSC_C_main->cache_hitpass += src->cache_hitpass);
# 101 "../../include/tbl/vsc_f_main.h"
(VSC_C_main->cache_miss += src->cache_miss);
# 110 "../../include/tbl/vsc_f_main.h"
;
...
;
# 150 "../../include/tbl/vsc_f_main.h"
(VSC_C_main->fetch_head += src->fetch_head);



(VSC_C_main->fetch_length += src->fetch_length);



(VSC_C_main->fetch_chunked += src->fetch_chunked);



(VSC_C_main->fetch_eof += src->fetch_eof);



(VSC_C_main->fetch_bad += src->fetch_bad);



(VSC_C_main->fetch_none += src->fetch_none);



(VSC_C_main->fetch_1xx += src->fetch_1xx);



(VSC_C_main->fetch_204 += src->fetch_204);



(VSC_C_main->fetch_304 += src->fetch_304);



(VSC_C_main->fetch_failed += src->fetch_failed);



(VSC_C_main->fetch_no_thread += src->fetch_no_thread);
# 201 "../../include/tbl/vsc_f_main.h"
;
...
(VSC_C_main->busy_sleep += src->busy_sleep);





(VSC_C_main->busy_wakeup += src->busy_wakeup);





(VSC_C_main->busy_killed += src->busy_killed);
...
(VSC_C_main->n_object += src->n_object);



(VSC_C_main->n_vampireobject += src->n_vampireobject);



(VSC_C_main->n_objectcore += src->n_objectcore);



(VSC_C_main->n_objecthead += src->n_objecthead);



(VSC_C_main->n_waitinglist += src->n_waitinglist);
...
(VSC_C_main->s_sess += src->s_sess);



(VSC_C_main->s_req += src->s_req);



(VSC_C_main->s_pipe += src->s_pipe);



(VSC_C_main->s_pass += src->s_pass);



(VSC_C_main->s_fetch += src->s_fetch);



(VSC_C_main->s_synth += src->s_synth);



(VSC_C_main->s_req_hdrbytes += src->s_req_hdrbytes);



(VSC_C_main->s_req_bodybytes += src->s_req_bodybytes);



(VSC_C_main->s_resp_hdrbytes += src->s_resp_hdrbytes);



(VSC_C_main->s_resp_bodybytes += src->s_resp_bodybytes);
...
(VSC_C_main->sess_closed += src->sess_closed);
...
(VSC_C_main->sess_readahead += src->sess_readahead);



(VSC_C_main->sess_herd += src->sess_herd);
# 406 "../../include/tbl/vsc_f_main.h"
# 1 "../../include/tbl/sess_close.h" 1
# 33 "../../include/tbl/sess_close.h"
;
...
(VSC_C_main->hcb_nolock += src->hcb_nolock);
...
Pool_Sumstat(struct worker *wrk)
{
 ...
 Lck__Lock(&wstat_mtx, __func__, 72);
 pool_sumstat(wrk->stats);
 Lck__Unlock(&wstat_mtx, __func__, 74);
...
{
 if (Lck__Trylock(&wstat_mtx, __func__, 81))
  return (0);
 pool_sumstat(wrk->stats);
 Lck__Unlock(&wstat_mtx, __func__, 84);
 memset(wrk->stats, 0, sizeof *wrk->stats);
 return (1);
}
...
Pool_Task_Any(struct pool_task *task, enum task_how how)
{
 ...
 Lck__Lock(&pool_mtx, __func__, 98);
 pp = ((&pools)->vtqh_first);
 if (pp != ((void *)0)) {
  do { if (((((pp))->list.vtqe_next)) != ((void *)0)) (((pp))->list.vtqe_next)->list.vtqe_prev = (pp)->list.vtqe_prev; else { (&pools)->vtqh_last = (pp)->list.vtqe_prev; } *(pp)->list.vtqe_prev = (((pp))->list.vtqe_next); ; ; } while (0);
  do { (((pp))->list.vtqe_next) = ((void *)0); (pp)->list.vtqe_prev = (&pools)->vtqh_last; *(&pools)->vtqh_last = (pp); (&pools)->vtqh_last = &(((pp))->list.vtqe_next); } while (0);
 }
 Lck__Unlock(&pool_mtx, __func__, 104);
 if (pp == ((void *)0))
  return (-1);
...
{
 Lck__Lock(&wstat_mtx, __func__, 118);
 VSC_C_main->n_purges++;
 VSC_C_main->n_obj_purged += nobj;
 Lck__Unlock(&wstat_mtx, __func__, 121);
}
...
pool_stat_summ(struct worker *wrk, void *priv)
{
 ...
 do { do { if (!((wrk) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_pool.c", 133, "(wrk) != NULL", VAS_ASSERT); } } while (0); do { if (!((wrk)->magic == 0x6391adcf)) { VAS_Fail(__func__, "cache/cache_pool.c", 133, "(wrk)->magic == 0x6391adcf", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((wrk->pool) != ((void *)0))) { VAS_Fail(__func__, "cache/cache_pool.c", 134, "(wrk->pool) != NULL", VAS_ASSERT); } } while (0); do { if (!((wrk->pool)->magic == 0x606658fa)) { VAS_Fail(__func__, "cache/cache_pool.c", 134, "(wrk->pool)->magic == 0x606658fa", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((priv) != 0)) { VAS_Fail(__func__, "cache/cache_pool.c", 135, "(priv) != 0", VAS_ASSERT); } } while (0); } while (0);
 src = priv;
 Lck__Lock(&wstat_mtx, __func__, 137);
 pool_sumstat(src);
 Lck__Unlock(&wstat_mtx, __func__, 139);
...
pool_mkpool(unsigned pool_no)
{
 ...
 do { (pp) = calloc(sizeof *(pp), 1); if ((pp) != ((void *)0)) (pp)->magic = (0x606658fa); } while (0);
 if (pp == ((void *)0))
  return (((void *)0));
 pp->a_stat = calloc(1, sizeof *pp->a_stat);
 do { do { if (!((pp->a_stat) != 0)) { VAS_Fail(__func__, "cache/cache_pool.c", 158, "(pp->a_stat) != 0", VAS_ASSERT); } } while (0); } while (0);
 pp->b_stat = calloc(1, sizeof *pp->b_stat);
 do { do { if (!((pp->b_stat) != 0)) { VAS_Fail(__func__, "cache/cache_pool.c", 160, "(pp->b_stat) != 0", VAS_ASSERT); } } while (0); } while (0);
 Lck__New(&pp->mtx, lck_wq, "lck_wq");

 do { (((&pp->idle_queue))->vtqh_first) = ((void *)0); (&pp->idle_queue)->vtqh_last = &(((&pp->idle_queue))->vtqh_first); } while (0);
 for (i = 0; i < TASK_QUEUE_END; i++)
  do { (((&pp->queues[i]))->vtqh_first) = ((void *)0); (&pp->queues[i])->vtqh_last = &(((&pp->queues[i]))->vtqh_first); } while (0);
 do { do { if (!((pthread_cond_init(&pp->herder_cond, ((void *)0))) == 0)) { VAS_Fail(__func__, "cache/cache_pool.c", 166, "(pthread_cond_init(&pp->herder_cond, ((void *)0))) == 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((pthread_create(&pp->herder_thr, ((void *)0), pool_herder, pp)) == 0)) { VAS_Fail(__func__, "cache/cache_pool.c", 167, "(pthread_create(&pp->herder_thr, ((void *)0), pool_herder, pp)) == 0", VAS_ASSERT); } } while (0); } while (0);
...
pool_poolherder(void *priv)
{
 ...
 while (1) {
  if (nwq < cache_param->wthread_pools) {
   pp = pool_mkpool(nwq);
   if (pp != ((void *)0)) {
    Lck__Lock(&pool_mtx, __func__, 201);
    do { (((pp))->list.vtqe_next) = ((void *)0); (pp)->list.vtqe_prev = (&pools)->vtqh_last; *(&pools)->vtqh_last = (pp); (&pools)->vtqh_last = &(((pp))->list.vtqe_next); } while (0);
    Lck__Unlock(&pool_mtx, __func__, 203);
    VSC_C_main->pools++;
    nwq++;
    continue;
   }
  ...
  if (0) {
   Lck__Lock(&pool_mtx, __func__, 211);
   pp = ((&pools)->vtqh_first);
   do { if (((((pp))->list.vtqe_next)) != ((void *)0)) (((pp))->list.vtqe_next)->list.vtqe_prev = (pp)->list.vtqe_prev; else { (&pools)->vtqh_last = (pp)->list.vtqe_prev; } *(pp)->list.vtqe_prev = (((pp))->list.vtqe_next); ; ; } while (0);
   Lck__Unlock(&pool_mtx, __func__, 214);
   do { do { if (!((pp) != 0)) { VAS_Fail(__func__, "cache/cache_pool.c", 215, "(pp) != 0", VAS_ASSERT); } } while (0); } while (0);
   MPL_Destroy(&pp->mpl_sess);
   MPL_Destroy(&pp->mpl_req);
   do { VAS_Fail(__func__, "cache/cache_pool.c", 218, "", VAS_INCOMPLETE); } while (0);
  }
  (void)sleep(1);
  u = 0;
  Lck__Lock(&pool_mtx, __func__, 222);
  for ((pp) = (((&pools))->vtqh_first); (pp); (pp) = (((pp))->list.vtqe_next))
   u += pp->lqueue;
  Lck__Unlock(&pool_mtx, __func__, 225);
...
Pool_Init(void)
{
 ...
 Lck__New(&wstat_mtx, lck_wstat, "lck_wstat");
 Lck__New(&pool_mtx, lck_wq, "lck_wq");
 do { do { if (!((pthread_create(&thr_pool_herder, ((void *)0), pool_poolherder, ((void *)0))) == 0)) { VAS_Fail(__func__, "cache/cache_pool.c", 239, "(pthread_create(&thr_pool_herder, ((void *)0), pool_poolherder, ((void *)0))) == 0", VAS_ASSERT); } } while (0); } while (0);
