...
VSL_Glob2Tags(const char *glob, int l, VSL_tagfind_f *func, void *priv)
{
 ...
 do { do { if (!((glob) != 0)) { VAS_Fail(__func__, "vsl_arg.c", 100, "(glob) != 0", VAS_ASSERT); } } while (0); } while (0);
 if (l < 0)
  l = strlen(glob);
 if (l == 0 || l > sizeof buf - 1)
  return (-1);
 if ((__extension__ (__builtin_constant_p ('*') && !__builtin_constant_p (glob) && ('*') == '\0' ? (char *) __rawmemchr (glob, '*') : __builtin_strchr (glob, '*'))) != ((void *)0)) {
  if (glob[0] == '*') {
 ...
 memcpy(buf, glob, l);
 buf[l] = '\0';
 if ((__extension__ (__builtin_constant_p ('*') && !__builtin_constant_p (buf) && ('*') == '\0' ? (char *) __rawmemchr (buf, '*') : __builtin_strchr (buf, '*'))) != ((void *)0))
...
vsl_ix_arg(struct VSL_data *vsl, int opt, const char *arg)
{
 ...
 do { do { if (!((vsl) != ((void *)0))) { VAS_Fail(__func__, "vsl_arg.c", 240, "(vsl) != NULL", VAS_ASSERT); } } while (0); do { if (!((vsl)->magic == 0x8E6C92AA)) { VAS_Fail(__func__, "vsl_arg.c", 240, "(vsl)->magic == 0x8E6C92AA", VAS_ASSERT); } } while (0); } while (0);
 vsl->flags |= (1 << 0);
...
vsl_IX_arg(struct VSL_data *vsl, int opt, const char *arg)
{
 ...
 vre_t *vre;
 struct vslf *vslf;
 struct vbitmap *tags = ((void *)0);

 do { do { if (!((vsl) != ((void *)0))) { VAS_Fail(__func__, "vsl_arg.c", 267, "(vsl) != NULL", VAS_ASSERT); } } while (0); do { if (!((vsl)->magic == 0x8E6C92AA)) { VAS_Fail(__func__, "vsl_arg.c", 267, "(vsl)->magic == 0x8E6C92AA", VAS_ASSERT); } } while (0); } while (0);
 vsl->flags |= (1 << 0);

 b = arg;
 e = (__extension__ (__builtin_constant_p (':') && !__builtin_constant_p (b) && (':') == '\0' ? (char *) __rawmemchr (b, ':') : __builtin_strchr (b, ':')));
 if (e) {
  tags = vbit_init(256);
  do { do { if (!((tags) != 0)) { VAS_Fail(__func__, "vsl_arg.c", 274, "(tags) != 0", VAS_ASSERT); } } while (0); } while (0);
  l = e - b;
 ...
 do { (vslf) = calloc(sizeof *(vslf), 1); if ((vslf) != ((void *)0)) (vslf)->magic = (0x08650B39); } while (0);
 do { do { if (!((vslf) != 0)) { VAS_Fail(__func__, "vsl_arg.c", 303, "(vslf) != 0", VAS_ASSERT); } } while (0); } while (0);
 vslf->tags = tags;
 vslf->vre = vre;

 if (opt == 'I')
  do { (((vslf))->list.vtqe_next) = ((void *)0); (vslf)->list.vtqe_prev = (&vsl->vslf_select)->vtqh_last; *(&vsl->vslf_select)->vtqh_last = (vslf); (&vsl->vslf_select)->vtqh_last = &(((vslf))->list.vtqe_next); } while (0);
 else {
  do { if (!(opt == 'X')) { VAS_Fail(__func__, "vsl_arg.c", 310, "opt == 'X'", VAS_ASSERT); } } while (0);
  do { (((vslf))->list.vtqe_next) = ((void *)0); (vslf)->list.vtqe_prev = (&vsl->vslf_suppress)->vtqh_last; *(&vsl->vslf_suppress)->vtqh_last = (vslf); (&vsl->vslf_suppress)->vtqh_last = &(((vslf))->list.vtqe_next); } while (0);
 }
...
VSL_Arg(struct VSL_data *vsl, int opt, const char *arg)
{
 ...
 do { do { if (!((vsl) != ((void *)0))) { VAS_Fail(__func__, "vsl_arg.c", 325, "(vsl) != NULL", VAS_ASSERT); } } while (0); do { if (!((vsl)->magic == 0x8E6C92AA)) { VAS_Fail(__func__, "vsl_arg.c", 325, "(vsl)->magic == 0x8E6C92AA", VAS_ASSERT); } } while (0); } while (0);

 if ((opt == 'i' || opt == 'I') && !(vsl->flags & (1 << 0)))
  for (i = 0; i < 256; i++)
