...
# 1 "<command-line>" 2
# 1 "http1/cache_http1_deliver.c"
# 30 "http1/cache_http1_deliver.c"
# 1 "../../config.h" 1
# 31 "http1/cache_http1_deliver.c" 2
...
# 33 "http1/cache_http1_deliver.c" 2
...
vdp_bytes VDP_ESI;
# 34 "http1/cache_http1_deliver.c" 2
...
# 35 "http1/cache_http1_deliver.c" 2
    ...
    const void *ptr, ssize_t len)
{
 ...
 do { do { if (!((req) != ((void *)0))) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 44, "(req) != NULL", VAS_ASSERT); } } while (0); do { if (!((req)->magic == 0x2751aaa1)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 44, "(req)->magic == 0x2751aaa1", VAS_ASSERT); } } while (0); } while (0);
 (void)priv;
 if (act == VDP_INIT || act == VDP_FINI)
  return (0);

 do { do { if (!((req->vdp_nxt) == 0)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 49, "(req->vdp_nxt) == 0", VAS_ASSERT); } } while (0); } while (0);
...
V1D_Deliver(struct req *req, struct busyobj *bo, int sendbody)
{
 ...
 do { do { if (!((req) != ((void *)0))) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 86, "(req) != NULL", VAS_ASSERT); } } while (0); do { if (!((req)->magic == 0x2751aaa1)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 86, "(req)->magic == 0x2751aaa1", VAS_ASSERT); } } while (0); } while (0);
 do { if ((bo) != ((void *)0)) do { if (!((bo)->magic == 0x23b95567)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 87, "(bo)->magic == 0x23b95567", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((req->objcore) != ((void *)0))) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 88, "(req->objcore) != NULL", VAS_ASSERT); } } while (0); do { if (!((req->objcore)->magic == 0x4d301302)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 88, "(req->objcore)->magic == 0x4d301302", VAS_ASSERT); } } while (0); } while (0);

 if (sendbody) {
  if (http_GetHdr(req->resp, H_Content_Length, ((void *)0)))
   req->res_mode |= (1<<1);
 ...
 } else if (!http_GetHdr(req->resp, H_Connection, ((void *)0)))
  http_SetHeader(req->resp, "Connection: keep-alive");

 if (sendbody && req->resp_len != 0)
  VDP_push(req, v1d_bytes, ((void *)0), 1);

 do { do { if (!((req->wrk->v1l) == 0)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 115, "(req->wrk->v1l) == 0", VAS_ASSERT); } } while (0); } while (0);
 V1L_Reserve(req->wrk, req->ws, &req->sp->fd, req->vsl, req->t_prev);

 if (WS_Overflowed(req->ws)) {
  v1d_error(req, "workspace_client overflow");
  do { do { if (!((req->wrk->v1l) == 0)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 120, "(req->wrk->v1l) == 0", VAS_ASSERT); } } while (0); } while (0);
  return;
 }
 ...
 if (sendbody && req->resp_len != 0) {
  ...
  if (!err && (req->res_mode & (1<<3)))
   V1L_EndChunk(req->wrk);
 }

 if ((V1L_FlushRelease(req->wrk) || err) && req->sp->fd >= 0)
  SES_Close(req->sp, SC_REM_CLOSE);
 do { do { if (!((req->wrk->v1l) == 0)) { VAS_Fail(__func__, "http1/cache_http1_deliver.c", 138, "(req->wrk->v1l) == 0", VAS_ASSERT); } } while (0); } while (0);
 VDP_close(req);
}
