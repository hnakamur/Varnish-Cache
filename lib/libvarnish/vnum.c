# 1 "vnum.c"
# 1 "/builddir/build/BUILD/Varnish-Cache-37d738ea4c04629766e510daf3d440ac621d8156/lib/libvarnish//"
# 1 "<built-in>"
# 1 "<command-line>"
...
VNUMpfx(const char *p, const char **t)
{
 ...
 do { do { if (!((p) != 0)) { VAS_Fail(__func__, "vnum.c", 59, "(p) != 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((t) != 0)) { VAS_Fail(__func__, "vnum.c", 60, "(t) != 0", VAS_ASSERT); } } while (0); } while (0);
 *t = ((void *)0);
 ...
 if (*p == 'e' || *p == 'E') {
  ...
  if (!((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit))
   return (nan(""));
  for (; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); p++)
   ee = ee * 10. + *p - '0';
 }
 while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace))
  p++;
