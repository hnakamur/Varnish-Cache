    ...
    const char **ip, const char **ip_ascii, const char **p_ascii)
{
 ...
 if (ip_ascii != ((void *)0))
  *ip_ascii = TlDup(tl, a);
 if (p_ascii != ((void *)0) && *p_ascii == ((void *)0))
  *p_ascii = TlDup(tl, p);
 ...
 q = TlAlloc(tl, 40);
 do { do { if (!((q) != 0)) { VAS_Fail(__func__, "vcc_utils.c", 126, "(q) != 0", VAS_ASSERT); } } while (0); } while (0);
...
rs_callback(void *priv, const struct suckaddr *vsa)
{
 ...
 do { (rss) = (priv); do { if (!((rss) != ((void *)0))) { VAS_Fail(__func__, "vcc_utils.c", 162, "(rss) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((rss))->magic == (0x11e966ab))) { VAS_Fail(__func__, "vcc_utils.c", 162, "((rss))->magic == (0x11e966ab)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 do { if (!(VSA_Sane(vsa))) { VAS_Fail(__func__, "vcc_utils.c", 163, "VSA_Sane(vsa)", VAS_ASSERT); } } while (0);
 ...
 *ipv6 = ((void *)0);
 ...
 do { (rss) = calloc(sizeof *(rss), 1); if ((rss) != ((void *)0)) (rss)->magic = (0x11e966ab); } while (0);
 do { do { if (!((rss) != 0)) { VAS_Fail(__func__, "vcc_utils.c", 207, "(rss) != 0", VAS_ASSERT); } } while (0); } while (0);
 rss->vsb = VSB_new(((void *)0), ((void *)0), 0, 0x00000001);
 do { do { if (!((rss->vsb) != 0)) { VAS_Fail(__func__, "vcc_utils.c", 209, "(rss->vsb) != 0", VAS_ASSERT); } } while (0); } while (0);

 error = VSS_resolver(host, def_port, rs_callback, rss, &err);
 do { do { if (!((VSB_finish(rss->vsb)) == 0)) { VAS_Fail(__func__, "vcc_utils.c", 212, "(VSB_finish(rss->vsb)) == 0", VAS_ASSERT); } } while (0); } while (0);
 if (err != ((void *)0)) {
  VSB_printf(tl->sb,
      "%s '%.*s' could not be resolved to an IP address:\n"
      "\t%s\n"
      "(Sorry if that error message is gibberish.)\n",
      errid, (int)((t_err)->e - (t_err)->b), (t_err)->b, err);
  vcc_ErrWhere(tl, t_err);
  free(rss->vsa4);
  free(rss->vsa6);
  VSB_delete(rss->vsb);
  do { (rss)->magic = (0); free(rss); rss = ((void *)0); } while (0);
  return;
 }
 do { do { if (!((error) == 0)) { VAS_Fail(__func__, "vcc_utils.c", 226, "(error) == 0", VAS_ASSERT); } } while (0); } while (0);
 if (rss->vsa4 != ((void *)0)) {
 ...
 if (rss->retval == 0) {
  VSB_printf(tl->sb,
      ...
      errid, (int)((t_err)->e - (t_err)->b), (t_err)->b );
  vcc_ErrWhere(tl, t_err);
 }
