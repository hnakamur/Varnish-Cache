...
HSH_config(const char *h_arg)
{
 ...
 do { do { if (!(getpid() == mgt_pid)) { VAS_Fail(__func__, "hash/mgt_hash.c", 61, "getpid() == mgt_pid", VAS_ASSERT); } } while (0);} while (0);
 av = VAV_Parse(h_arg, ((void *)0), (1 << 1));
 do { do { if (!((av) != 0)) { VAS_Fail(__func__, "hash/mgt_hash.c", 63, "(av) != 0", VAS_ASSERT); } } while (0); } while (0);

 if (av[0] != ((void *)0))
  do { fprintf(stderr, "Error: " "%s\n", av[0]); exit(2); } while (0);

 if (av[1] == ((void *)0))
  do { fprintf(stderr, "Error: " "-h argument is empty\n"); exit(2); } while (0);

 for (ac = 0; av[ac + 2] != ((void *)0); ac++)
  continue;

 hp = pick(hsh_choice, av[1], "hash");
 do { do { if (!((hp) != ((void *)0))) { VAS_Fail(__func__, "hash/mgt_hash.c", 75, "(hp) != NULL", VAS_ASSERT); } } while (0); do { if (!((hp)->magic == 0x1b720cba)) { VAS_Fail(__func__, "hash/mgt_hash.c", 75, "(hp)->magic == 0x1b720cba", VAS_ASSERT); } } while (0); } while (0);
 VSB_printf(vident, ",-h%s", av[1]);
 heritage.hash = hp;
 if (hp->init != ((void *)0))
  hp->init(ac, av + 2);
 else if (ac > 0)
  do { fprintf(stderr, "Error: " "Hash method \"%s\" takes no arguments\n", hp->name); exit(2); } while (0)
               ;
