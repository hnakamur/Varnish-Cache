...
vcc_Var_Wildcard(struct vcc *tl, const struct token *t, const struct symbol *wc)
{
 ...
 vh = wc->var;
 do { if (!(vh->fmt == HEADER)) { VAS_Fail(__func__, "vcc_var.c", 51, "vh->fmt == HEADER", VAS_ASSERT); } } while (0);

 v = TlAlloc(tl, sizeof *v);
 do { do { if (!((v) != 0)) { VAS_Fail(__func__, "vcc_var.c", 54, "(v) != 0", VAS_ASSERT); } } while (0); } while (0);
 ...
 do { do { if (!((vsb) != 0)) { VAS_Fail(__func__, "vcc_var.c", 63, "(vsb) != 0", VAS_ASSERT); } } while (0); } while (0);
 VSB_printf(vsb, "&VGC_%s_", vh->rname);
 for (p = leaf, u = 1; *p != '\0'; p++, u++)
  if (vct_is(*p, (1<<3)) || vct_is(*p, (1<<5)))
   VSB_putc(vsb, *p);
  else
   VSB_printf(vsb, "_%02x_", *p);
 do { do { if (!((VSB_finish(vsb)) == 0)) { VAS_Fail(__func__, "vcc_var.c", 70, "(VSB_finish(vsb)) == 0", VAS_ASSERT); } } while (0); } while (0);
 ...
 do { do { if (!((VSB_finish(vsb)) == 0)) { VAS_Fail(__func__, "vcc_var.c", 80, "(VSB_finish(vsb)) == 0", VAS_ASSERT); } } while (0); } while (0);
 v->lname = TlDup(tl, VSB_data(vsb));
 VSB_delete(vsb);

 sym = VCC_AddSymbolTok(tl, t, SYM_VAR);
 do { do { if (!((sym) != 0)) { VAS_Fail(__func__, "vcc_var.c", 85, "(sym) != 0", VAS_ASSERT); } } while (0); } while (0);
 sym->var = v;
 sym->fmt = v->fmt;
    ...
    const char *use)
{
 ...
 do { do { if (!((tl->vars) != 0)) { VAS_Fail(__func__, "vcc_var.c", 102, "(tl->vars) != 0", VAS_ASSERT); } } while (0); } while (0);
 sym = VCC_FindSymbol(tl, t, SYM_VAR);
 if (sym != ((void *)0)) {
  v = sym->var;
  do { do { if (!((v) != 0)) { VAS_Fail(__func__, "vcc_var.c", 106, "(v) != 0", VAS_ASSERT); } } while (0); } while (0);
