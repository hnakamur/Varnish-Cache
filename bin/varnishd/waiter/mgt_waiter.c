...
# 1 "<command-line>" 2
# 1 "waiter/mgt_waiter.c"
# 31 "waiter/mgt_waiter.c"
# 1 "../../config.h" 1
# 32 "waiter/mgt_waiter.c" 2
...
Wait_config(const char *arg)
{
 ...
 do { do { if (!(getpid() == mgt_pid)) { VAS_Fail(__func__, "waiter/mgt_waiter.c", 59, "getpid() == mgt_pid", VAS_ASSERT); } } while (0);} while (0);
