...
vxp_add_token(struct vxp *vxp, unsigned tok, const char *b, const char *e)
{
 ...
 do { do { if (!((t) != 0)) { VAS_Fail(__func__, "vxp_lexer.c", 53, "(t) != 0", VAS_ASSERT); } } while (0); } while (0);
 t->tok = tok;
 t->b = b;
 t->e = e;
 if (vxp->t != ((void *)0))
  do { if (((((t))->list.vtqe_next) = (((vxp->t))->list.vtqe_next)) != ((void *)0)) (((t))->list.vtqe_next)->list.vtqe_prev = &(((t))->list.vtqe_next); else { (&vxp->tokens)->vtqh_last = &(((t))->list.vtqe_next); } (((vxp->t))->list.vtqe_next) = (t); (t)->list.vtqe_prev = &(((vxp->t))->list.vtqe_next); } while (0);
 else
  do { (((t))->list.vtqe_next) = ((void *)0); (t)->list.vtqe_prev = (&vxp->tokens)->vtqh_last; *(&vxp->tokens)->vtqh_last = (t); (&vxp->tokens)->vtqh_last = &(((t))->list.vtqe_next); } while (0);
 vxp->t = t;
}
...
vxp_decstr(struct vxp *vxp, int quoted)
{
 ...
 do { if (!(vxp->t->tok == 140)) { VAS_Fail(__func__, "vxp_lexer.c", 72, "vxp->t->tok == VAL", VAS_ASSERT); } } while (0);

 b = vxp->t->b;
 e = vxp->t->e;
 if (quoted) {
  do { if (!(e - b >= 2)) { VAS_Fail(__func__, "vxp_lexer.c", 77, "e - b >= 2", VAS_ASSERT); } } while (0);
  b++;
  e--;
 }
 l = e - b;
 s = vxp->t->dec = vxp_Alloc(vxp, l + 1);
 do { do { if (!((vxp->t->dec) != 0)) { VAS_Fail(__func__, "vxp_lexer.c", 83, "(vxp->t->dec) != 0", VAS_ASSERT); } } while (0); } while (0);
 for (p = b; p < e; p++) {
  if (!esc && *p == '\\') {
   esc = 1;
   continue;
...
vxp_Lexer(struct vxp *vxp)
{
 ...
 for (p = vxp->b; p < vxp->e; ) {
  ...
  if (u != 0) {
   do { do { if (!((q) != 0)) { VAS_Fail(__func__, "vxp_lexer.c", 123, "(q) != 0", VAS_ASSERT); } } while (0); } while (0);
  ...
  if ((((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISalpha) || ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit) || (*p) == '_' || (*p) == '-' || (*p) == '+' || (*p) == '.' || (*p) == '*')) {
   for (q = p; q < vxp->e; q++)
    if (!(((*__ctype_b_loc ())[(int) ((*q))] & (unsigned short int) _ISalpha) || ((*__ctype_b_loc ())[(int) ((*q))] & (unsigned short int) _ISdigit) || (*q) == '_' || (*q) == '-' || (*q) == '+' || (*q) == '.' || (*q) == '*'))
     break;
   vxp_add_token(vxp, 140, p, q);
   if (vxp_decstr(vxp, 0))
    return;
 ...
 vxp_add_token(vxp, 128, vxp->e, vxp->e);
}
...
vxp_PrintTokens(const struct vxp *vxp)
{
 ...
 for ((t) = (((&vxp->tokens))->vtqh_first); (t); (t) = (((t))->list.vtqe_next)) {
  fprintf(stderr, "  ");
