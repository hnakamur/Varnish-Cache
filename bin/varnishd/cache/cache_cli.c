...
# 1 "<command-line>" 2
# 1 "cache/cache_cli.c"
# 37 "cache/cache_cli.c"
# 1 "../../config.h" 1
# 38 "cache/cache_cli.c" 2
...
# 46 "cache/cache_cli.c" 2

pthread_t cli_thread;
static struct lock cli_mtx;
static int add_check;
static struct VCLS *cls;
# 64 "cache/cache_cli.c"
void
CLI_AddFuncs(struct cli_proto *p)
{

 do { do { if (!((add_check) == 0)) { VAS_Fail(__func__, "cache/cache_cli.c", 68, "(add_check) == 0", VAS_ASSERT); } } while (0); } while (0);
 Lck__Lock(&cli_mtx, __func__, 69);
 do { do { if (!((VCLS_AddFunc(cls, 0, p)) == 0)) { VAS_Fail(__func__, "cache/cache_cli.c", 70, "(VCLS_AddFunc(cls, 0, p)) == 0", VAS_ASSERT); } } while (0); } while (0);
 Lck__Unlock(&cli_mtx, __func__, 71);
}
...
cli_cb_before(const struct cli *cli)
{
 ...
 do {do { if (!(pthread_self() == cli_thread)) { VAS_Fail(__func__, "cache/cache_cli.c", 78, "pthread_self() == cli_thread", VAS_ASSERT); } } while (0);} while (0);
 VSL(SLT_CLI, 0, "Rd %s", cli->cmd);
 VCL_Poll();
 VBE_Poll();
 Lck__Lock(&cli_mtx, __func__, 82);
}
...
cli_cb_after(const struct cli *cli)
{
 ...
 do {do { if (!(pthread_self() == cli_thread)) { VAS_Fail(__func__, "cache/cache_cli.c", 89, "pthread_self() == cli_thread", VAS_ASSERT); } } while (0);} while (0);
 Lck__Unlock(&cli_mtx, __func__, 90);
...
CLI_Run(void)
{
 ...
 do { do { if (!((VCLS_AddFd(cls, heritage.cli_in, heritage.cli_out, ((void *)0), ((void *)0))) != 0)) { VAS_Fail(__func__, "cache/cache_cli.c", 102, "(VCLS_AddFd(cls, heritage.cli_in, heritage.cli_out, ((void *)0), ((void *)0))) != 0", VAS_ASSERT); } } while (0); } while (0);
...
static struct cli_proto cli_cmds[] = {
 { "ping", "ping [<timestamp>]", "\tKeep connection alive.", 0, 1, "i", VCLS_func_ping },
 { "help", "help [<command>]", "\tShow command/protocol help.", 0, 1, "i", VCLS_func_help },
 { ((void *)0) }
};
...
CLI_Init(void)
{
 ...
 Lck__New(&cli_mtx, lck_cli, "lck_cli");
 cli_thread = pthread_self();

 cls = VCLS_New(cli_cb_before, cli_cb_after,
     &cache_param->cli_buffer, &cache_param->cli_limit);
 do { do { if (!((cls) != 0)) { VAS_Fail(__func__, "cache/cache_cli.c", 132, "(cls) != 0", VAS_ASSERT); } } while (0); } while (0);
