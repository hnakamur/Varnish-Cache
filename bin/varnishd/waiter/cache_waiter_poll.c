...
# 1 "<command-line>" 2
# 1 "waiter/cache_waiter_poll.c"
# 31 "waiter/cache_waiter_poll.c"
# 1 "../../config.h" 1
# 32 "waiter/cache_waiter_poll.c" 2
...
# 34 "waiter/cache_waiter_poll.c" 2
...
# 35 "waiter/cache_waiter_poll.c" 2
...
# 36 "waiter/cache_waiter_poll.c" 2
...
# 38 "waiter/cache_waiter_poll.c" 2
...
# 40 "waiter/cache_waiter_poll.c" 2
...
# 41 "waiter/cache_waiter_poll.c" 2
# 1 "../../include/vtim.h" 1
# 33 "../../include/vtim.h"
...
# 42 "waiter/cache_waiter_poll.c" 2
...
vwp_extend_pollspace(struct vwp *vwp)
{
 ...
 do { do { if (!((vwp->pollfd) != 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 81, "(vwp->pollfd) != 0", VAS_ASSERT); } } while (0); } while (0);
 memset(vwp->pollfd + vwp->npoll, 0, inc * sizeof(*vwp->pollfd));

 vwp->idx = realloc(vwp->idx, (vwp->npoll + inc) * sizeof(*vwp->idx));
 do { do { if (!((vwp->idx) != 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 85, "(vwp->idx) != 0", VAS_ASSERT); } } while (0); } while (0);
...
VSL(SLT_Debug, wp->fd, "ADD");
 do { do { if (!((wp) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 99, "(wp) != NULL", VAS_ASSERT); } } while (0); do { if (!((wp)->magic == 0x1743992d)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 99, "(wp)->magic == 0x1743992d", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((vwp) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 100, "(vwp) != NULL", VAS_ASSERT); } } while (0); do { if (!((vwp)->magic == 0x4b2cc735)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 100, "(vwp)->magic == 0x4b2cc735", VAS_ASSERT); } } while (0); } while (0);
 if (vwp->hpoll == vwp->npoll)
  vwp_extend_pollspace(vwp);
 do { if (!(vwp->hpoll < vwp->npoll)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 103, "vwp->hpoll < vwp->npoll", VAS_ASSERT); } } while (0);
 do { if (!(vwp->pollfd[vwp->hpoll].fd == -1)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 104, "vwp->pollfd[vwp->hpoll].fd == -1", VAS_ASSERT); } } while (0);
 do { do { if (!((vwp->idx[vwp->hpoll]) == 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 105, "(vwp->idx[vwp->hpoll]) == 0", VAS_ASSERT); } } while (0); } while (0);
 vwp->pollfd[vwp->hpoll].fd = wp->fd;
 vwp->pollfd[vwp->hpoll].events = 0x001;
...
vwp_dopipe(struct vwp *vwp)
{
 ...
 do { if (!(ss > 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 137, "ss > 0", VAS_ASSERT); } } while (0);
 i = 0;
 while (ss) {
  if (w[i] == ((void *)0)) {
   do { if (!(ss == sizeof w[0])) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 141, "ss == sizeof w[0]", VAS_ASSERT); } } while (0);
   do { if (!(vwp->hpoll == 1)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 142, "vwp->hpoll == 1", VAS_ASSERT); } } while (0);
   pthread_exit(((void *)0));
  }
  do { do { if (!((w[i]) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 145, "(w[i]) != NULL", VAS_ASSERT); } } while (0); do { if (!((w[i])->magic == 0x1743992d)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 145, "(w[i])->magic == 0x1743992d", VAS_ASSERT); } } while (0); } while (0);
  do { if (!(w[i]->fd > 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 146, "w[i]->fd > 0", VAS_ASSERT); } } while (0);
  vwp_add(vwp, w[i++]);
  ss -= sizeof w[0];
 }
}
...
vwp_main(void *priv)
{
 ...
 do { (vwp) = (priv); do { if (!((vwp) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 165, "(vwp) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((vwp))->magic == (0x4b2cc735))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 165, "((vwp))->magic == (0x4b2cc735)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 w = vwp->waiter;
 ...
 while (1) {
  ...
  do { if (!(vwp->hpoll > 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 174, "vwp->hpoll > 0", VAS_ASSERT); } } while (0);
  do { do { if (!((vwp->pollfd) != 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 175, "(vwp->pollfd) != 0", VAS_ASSERT); } } while (0); } while (0);
  v = poll(vwp->pollfd, vwp->hpoll, i);
  do { if (!(v >= 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 177, "v >= 0", VAS_ASSERT); } } while (0);
  now = VTIM_real();
  if (vwp->pollfd[0].revents)
   v--;
  for (i = 1; i < vwp->hpoll;) {
VSL(SLT_Debug, vwp->pollfd[i].fd, "POLL loop i=%d revents=0x%x", i, vwp->pollfd[i].revents);
   do { if (!(vwp->pollfd[i].fd != vwp->pipes[0])) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 183, "vwp->pollfd[i].fd != vwp->pipes[0]", VAS_ASSERT); } } while (0);
   wp = vwp->idx[i];
   do { do { if (!((wp) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 185, "(wp) != NULL", VAS_ASSERT); } } while (0); do { if (!((wp)->magic == 0x1743992d)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 185, "(wp)->magic == 0x1743992d", VAS_ASSERT); } } while (0); } while (0);

   if (v == 0 && Wait_HeapDue(w, ((void *)0)) > now)
    break;
   ...
   if (then <= now) {
    ...
    vwp_del(vwp, i);
   } else if (vwp->pollfd[i].revents & 0x001) {
    do { if (!(wp->fd > 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 197, "wp->fd > 0", VAS_ASSERT); } } while (0);
    do { if (!(wp->fd == vwp->pollfd[i].fd)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 198, "wp->fd == vwp->pollfd[i].fd", VAS_ASSERT); } } while (0);
    Wait_HeapDelete(w, wp);
...
vwp_enter(void *priv, struct waited *wp)
{
 ...
 do { (vwp) = (priv); do { if (!((vwp) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 219, "(vwp) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((vwp))->magic == (0x4b2cc735))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 219, "((vwp))->magic == (0x4b2cc735)", VAS_ASSERT); } } while (0); } while (0); } while (0);
...
vwp_init(struct waiter *w)
{
 ...
 do { do { if (!((w) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 233, "(w) != NULL", VAS_ASSERT); } } while (0); do { if (!((w)->magic == 0x17c399db)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 233, "(w)->magic == 0x17c399db", VAS_ASSERT); } } while (0); } while (0);
 vwp = w->priv;
 do { (void)memset(vwp, 0, sizeof *vwp); (vwp)->magic = (0x4b2cc735); } while (0);
 vwp->waiter = w;
 do { do { if (!((pipe(vwp->pipes)) == 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 237, "(pipe(vwp->pipes)) == 0", VAS_ASSERT); } } while (0); } while (0);


 vwp->hpoll = 1;
 vwp_extend_pollspace(vwp);
 vwp->pollfd[0].fd = vwp->pipes[0];
 vwp->pollfd[0].events = 0x001;
 do { do { if (!((pthread_create(&vwp->thread, ((void *)0), vwp_main, vwp)) == 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 244, "(pthread_create(&vwp->thread, ((void *)0), vwp_main, vwp)) == 0", VAS_ASSERT); } } while (0); } while (0);
}
...
vwp_fini(struct waiter *w)
{
 ...
 do { (vwp) = (w->priv); do { if (!((vwp) != ((void *)0))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 258, "(vwp) != NULL", VAS_ASSERT); } } while (0); do { do { if (!(((vwp))->magic == (0x4b2cc735))) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 258, "((vwp))->magic == (0x4b2cc735)", VAS_ASSERT); } } while (0); } while (0); } while (0);
 vp = ((void *)0);
 while (vwp->hpoll > 1)
  (void)usleep(100000);

 do { if (!(write(vwp->pipes[1], &vp, sizeof vp) == sizeof vp)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 263, "write(vwp->pipes[1], &vp, sizeof vp) == sizeof vp", VAS_ASSERT); } } while (0);
 do { do { if (!((pthread_join(vwp->thread, &vp)) == 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 264, "(pthread_join(vwp->thread, &vp)) == 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((close(vwp->pipes[0])) == 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 265, "(close(vwp->pipes[0])) == 0", VAS_ASSERT); } } while (0); } while (0);
 do { do { if (!((close(vwp->pipes[1])) == 0)) { VAS_Fail(__func__, "waiter/cache_waiter_poll.c", 266, "(close(vwp->pipes[1])) == 0", VAS_ASSERT); } } while (0); } while (0);
 free(vwp->pollfd);
 free(vwp->idx);
}
